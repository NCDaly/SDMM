	...
	if (me == 0) {
		...
		while (inv->prog < inv->size) {
			...
			if (pe < npes) {
				...
				shmem_T_put(A_t.data, Ai.data, (Ai.rows*Ai.cols), pe);
				...
				sendingData.start = clock();
               		        shmem_T_put(B_t.data, Bi.data, (Bi.rows*Bi.cols), pe);
			        sendingData.end = clock();
	               	        sendingData.total += (double)(sendingData.end - sendingData.start);

				//Set start flag on node to start
				shmem_int_p(&flagStart, 1, pe);
			...
			if (shmem_int64_test(&PE_done, SHMEM_CMP_NE, 0)) {
				for (i = 1; i < npes; i++) {
       	 	                        if (PE_done & (1LU << i)) {
