...
VdmInverter *vdm_inv_init(int size, int alpha_rng) {
  ...
  inv->numer = (int *) calloc(size * (size+1), sizeof(int));
...
int vdm_inv_update(VdmInverter *inv, int new) {
  ...
  // Final update: change numer matrix to store actual value (numer/denom)
  if (inv->prog == inv->size) {
    for (int i = 0; i < inv->size; i++) {
      int offset = i * inv->size;
      for (int j = 0; j < inv->size; j++) {
	inv->numer[offset + j] = mpf_mul(inv->numer[offset + j], inv->denom[j]);
      }
    }
  }
...
TYPE vdm_inv_eval(VdmInverter *inv, TYPE *data, int i) {
  ...
  for (int j = 0; j < inv->size; j++) {
    ...
    int new_term = mpf_mul(inv->numer[ij++], (int) data[j]);
