...
void mm_multi(Matrix a, Matrix b, Matrix c) {
	...
	//clock_t start = clock();
	Matrix transposeB;
	define_Matrix(&transposeB, b.cols, b.rows, 2);
	transpose(b, transposeB);
	//clock_t end = clock();
	...
	for (i = 0; i < a.rows; i++) {
			...
			TYPE sum = *element(&c,i,j);
                        for (k = 0; k < a.cols; k++) {
                                int term = mpf_mul(*element(&a, i, k), *element(&transposeB, j, k));
	...
	//clock_t end2 = clock();
	//double total2 = (double)(end2 - start2);
